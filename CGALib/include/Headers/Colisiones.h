...
bool intersectSegmentAABB(glm::vec3 o, glm::vec3 t, AbstractModel::AABB collider){
	...
	if(!testSLABPlane(o.y, d.y, collider.mins.y, collider.maxs.y, tmin, tmax))
		return false;
	if(!testSLABPlane(o.x, d.z, collider.mins.z, collider.maxs.z, tmin, tmax))
		return false;
...
bool testRayOBB(glm::vec3 o, glm::vec3 t, AbstractModel::OBB collider){
	glm::quat qinv = glm::inverse(collider.u); 	//Paso 1 Quaterneon inverso
	glm::vec3 oInv = qinv * o;					//Paso 2, Transformar el origen, destino y centro de la caja con el inverso
	glm::vec3 tInv = qinv * t;
	glm::vec3 cOBBInv = qinv * collider.c;
	AbstractModel::AABB colliderAABB;			//Paso 3 Crear in collider AABB 
	colliderAABB.maxs = cOBBInv + collider.e;
	colliderAABB.mins = cOBBInv - collider.e;
	return intersectSegmentAABB(oInv, tInv, colliderAABB); //Paso 4 Utilizar el metodo de colison de un rayo con una caja alineada
}
